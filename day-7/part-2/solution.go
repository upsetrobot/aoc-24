 ...
 *
 * For the second part, we just have to parse for the combination operator.
 *
 * file:        solution.go
...
func main() {
    ...
    for _, str := range strLines {
        ...
        for i := range int(math.Pow(3, float64(len(nums) - 2))) {
            testSumProduct := nums[0]
            sumProduct := nums[1]

            for j, val := range nums[2:] {
                iterVal := int(math.Pow(3, float64(j)))

                if i / iterVal % 3 == 0 {
                    sumProduct += val

                } else if i / iterVal % 3 == 1 {
                    sumProduct *= val

                } else {
                    tmp := strconv.Itoa(sumProduct) + strconv.Itoa(val)
                    sumProduct, err = strconv.Atoi(tmp)
                    if err != nil {
                        log.Fatal("Error converting string to number:", err)
                    }
                }
