 ...
 * Worked out the box.
 *
 * For part 2, the mission is to determine the first byte that blocks the path
 * to the exit. This is gonna be an easy one, I think. We just have run the path
 * algorithm with each falling byte. Then when it is max, that byte is the
 * soltuion.
 *
 * Worked out the box. Nice. But it is slow. Took about 1 minute 20 seconds on
 * my machine.
 *
 * file:        solution.go
...
func main() {
    ...
    // Part 2.
    // I'm gonna just trying shoving everything in a loop.
    ...
    fmt.Println()
    fmt.Println("Solution:", strconv.Itoa(solution.x) + "," + strconv.Itoa(solution.y))
    fmt.Println()
